...
DEFAULT_TRAINING_ARGS = {
    ...
    "minecraft_training_profile": False,
    "online_target_fps": 0.0,
    "online_use_vpt_camera_poses": False,
    "online_vpt_translation_scale": 0.1,
    "online_geometry_keyframe_stride": 1,
...
def _action_feature_vector(segment_stats):
    stats = dict(segment_stats or {})
    keys = [
        "move_forward",
        "move_backward",
        "move_left",
    ...
    for key in keys:
        ...
        if key in {"mouse_dx", "mouse_dy", "yaw_delta", "pitch_delta"}:
            value = max(min(value / 12.0, 1.0), -1.0)
        elif key == "speed":
            value = max(min(value / 300.0, 1.0), -1.0)
        else:
...
def _action_vector_to_history_bias(vector, template, *, phase_shift=0.0):
    if template is None or vector.numel() == 0 or template.numel() == 0:
        return template
    device = template.device
    dtype = template.dtype
    vector = vector.to(device=device, dtype=torch.float32)
    channels = int(template.shape[1])
    times = int(template.shape[2])
    channel_ids = torch.arange(channels, device=device, dtype=torch.float32)
    basis = []
    for index, value in enumerate(vector):
        frequency = ((channel_ids % float(index + 7)) + 1.0) / float(index + 7)
        wave = torch.sin((frequency + phase_shift) * float(index + 1))
        wave = wave + 0.5 * torch.cos((frequency * 0.5 + phase_shift) * float(index + 1))
        basis.append(wave * value)
    channel_bias = torch.stack(basis, dim=0).mean(dim=0).view(1, channels, 1, 1, 1)
    time_ramp = torch.linspace(0.75, 1.25, times, device=device, dtype=torch.float32).view(1, 1, times, 1, 1)
    return (channel_bias * time_ramp).to(dtype=dtype)
...
def assert_lora_only_trainable(
    ...
    *,
    ...
    if unexpected:
        raise ValueError(
            "Detected non-LoRA trainable transformer parameters in lightweight WAH training: "
            + ", ".join(unexpected[:20])
        )
